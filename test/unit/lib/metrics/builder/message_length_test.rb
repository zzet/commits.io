...
class Metrics::Builder::MessageLengthTest < ActiveSupport::TestCase
  def test_could_calculate_metric
    data = generate :metrics_message
    data["message"] = "Test message"

    metric = Metrics::Builder::MessageLength.new data
    metrics = metric.calculate
    assert_equal metrics, {:metric=>:message_length, :data=>{:length=>12, :rate=>0.6}}
  end
